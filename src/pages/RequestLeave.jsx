...
const RequestLeave = () => {
  ...
  useEffect(() => {
    const checkLeaveEligibility = async () => {
      try {
        ...
        if (response.data.joined_date) {
          const joinDate = new Date(response.data.joined_date);
  ...
  const handleSubmit = async (e) => {
    ...
    // Check if user is trying to take non-UPL leave within first 3 months
    if (!canTakePaidLeave && leaveData.leave_type !== "UPL") {
      setMessage("You can only take unpaid leave (UPL) during your first 3 months of employment.");
      return;
    }
    ...
    if (leaveData.leave_type === "ML" && medicalCert) {
      formData.append("medical_cert", medicalCert);
    }
    ...
    try {
      ...
      let errorMessage =
        "Failed to submit leave. Please check server status and network.";
      if (status === 400)
        errorMessage = `Submission Error: ${error.response.data.message || "Missing required fields."
          }`;
      else if (status === 500)
        errorMessage =
          "Server Error: The backend crashed while processing the request.";
            ...
            >
              ...
              {canTakePaidLeave ? (
                <>
                  ...
                  <option value="HML">Half Day Medical Leave</option>
                  <option value="HEL">Half Day Earned Leave</option>
                  <option value="HUPL">Half Day Unpaid Leave</option>
                </>
              ) : (
                <option value="UPL">Unpaid Leave (Only available during first 3 months)</option>
              )}
            </select>
          </div>
