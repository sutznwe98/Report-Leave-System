...
const fetchLeaves = useCallback(async () => {
  ...
  try {
    const leavesRes = await axios.get(`${API_URL}/leaves/employee/me`, {
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    try {
      const [employeeRes, myReportsRes, todayRes] = await Promise.all([
        axios.get(`${API_URL}/stats/employee/${userId}`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get(`${API_URL}/reports/employee/me`, {
          headers: { Authorization: `Bearer ${token}` },
          params: { id: userId }, // Pass user ID as param for this specific route
        }),
        axios
                ...
                .map((l) => {
                  ...
                  const days =
                    start && end
                      ...
                      : 0;
                  const status = l.pj_lead_status || l.status || "Pending";
